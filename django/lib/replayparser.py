...
try:
    f = open(os.path.join(folder, "tanks.json"), "r")
    tanks = json.load(f)
except Exception:
    raise
else:
...
try:
    f = open(os.path.join(folder, "maps.json"), "r")
    mapss = json.load(f)
except Exception:
    raise
else:
...
def replay(filename, to_decode):
            ...
            if second_chunk[0:2] == b'(d':
                ...
                if to_decode & 4:
                    ...
                    for b in third_chunk_decoded['vehicles']:
                        third_chunk_decoded['vehicles'][b]['details'] = _Decoder.decode_details(
                            third_chunk_decoded['vehicles'][b]['details'].encode('raw_unicode_escape'))
                        third_chunk_decoded['players'][third_chunk_decoded['vehicles'][b]['accountDBID']][
                            "vehicleid"] = b
                processing = 2;
                break
    ...
    if chunks_bitmask & 1 == 1:
        if "clientVersionFromExe" in first_chunk_decoded:
            version = int(first_chunk_decoded["clientVersionFromExe"].replace(', ', '').replace('.', ''))
...
def battle_result(filename):
    ...
    #   print(deind, val)
    common_decoded = {}
    common_decoded["arenaTypeID"] = common_to_decode[
        0]  # & 65535 makes more sense, but original replay keeps this number intact
